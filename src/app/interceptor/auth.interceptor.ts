...
export class AuthInterceptor implements HttpInterceptor {
  ...
  intercept(httpRequest: HttpRequest<any>, httpHandler: HttpHandler): Observable<HttpEvent<any>> {
    ...
    if (httpRequest.url.includes(`${this.authenticationService.host}/product`)) {
      return httpHandler.handle(httpRequest);
    }
    if (httpRequest.url.includes(`${this.authenticationService.host}/search`)) {
      return httpHandler.handle(httpRequest);
    }
    if (httpRequest.url.includes(`${this.authenticationService.host}/checkout`)) {
      return httpHandler.handle(httpRequest);
    }
    if (httpRequest.url.includes(`${this.authenticationService.host}/order/placeOrder`)) {
      return httpHandler.handle(httpRequest);
    }
